...
from options.train_options import ArgumentParser, get_timestamp
import os
...
def save_data(batch, folder, epoch): # TODO: extend it to n views, n is default 2, focusing on image pairs for now.
    ...
    for batch_idx in range(batch_size):
        ...
        _, _, Pinv0, _ = cam_batch0["P"][batch_idx],\
        ...
        _, _, Pinv1, _ = cam_batch1["P"][batch_idx],\
...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(os.listdir('./dataset')) > 0:
            print("./dataset/ exists and not empty. Aborted.")
            exit(-1)

    for epoch in range(opts.max_epoch):
        batch = next(iter(train_data_loader))
        save_data(batch, './dataset/', epoch)